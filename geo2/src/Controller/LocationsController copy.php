    ...
    public function autocomplete() {
        ...
        if(!empty($this->request->query['term'])) {
            ...
            foreach($terms as $term) {
                $conditions .= " AND name LIKE '%" . $term . "%' OR city LIKE '%" . $term . "%' " ;
            }
            $locations = $this->Locations->find('all', array(
                'fields' => array(
                    'Locations.id',
...
////////////////////////////////////////////////////////////
    ...
    public function search() {
        ...
        $loc = '';
        $map = '';
        $conditions = ' 1 = 1 AND Locations.active = 1 ';
        ...
        $this->set(compact('name', 'loc', 'unit', 'distance', 'search', 'map', 'locations'));
...
////////////////////////////////////////////////////////////
    ...
    public function liveupdate() {
        ...
        if(isset($this->request->query['southWestLat'])) {
            $conditions = array(
                'Locations.lat >=' => $this->request->query['southWestLat'],
                'Locations.lat <=' => $this->request->query['northEastLat'],
                'Locations.lng >=' => $this->request->query['southWestLng'],
                'Locations.lng <=' => $this->request->query['northEastLng']
            );
        }
        ...
        if(!empty($this->request->query['name'])) {
            $name = $this->request->query['name'];
            $name = preg_replace('/[^a-zA-Z0-9_\-\,\']/', '', $name);
            $conditions['Locations.name LIKE'] = "%$name%";
        }
        ...
        $locations = $this->Locations->find('all', array(
            ...
            'limit' => 100,
            'fields' => ["id", "name", "slug", "phone", "address", "city", "state", "postal_code", "lat", "lng", "distance" => "TRUNCATE((3958 * 3.1415926 * SQRT((`lat` - {$lat}) * (`lat` - {$lat}) + COS(`lat` / 57.29578) * COS({$lat} / 57.29578) * (`lng` - {$lng}) * (`lng` - {$lng})) / 180) * {$unit}, 1)"],
        ));
